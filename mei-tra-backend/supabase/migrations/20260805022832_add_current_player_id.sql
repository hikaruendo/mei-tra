...
begin
  ...
  where game_state.current_player_id is null;
  ...
  execute $sql$
    ...
    begin
      ...
      if current_state.current_player_id is not null then
        select player_position.zero_based_index
        into next_current_player_index
        from (
          select
            player_id,
            ordinality - 1 as zero_based_index
          from jsonb_array_elements_text(
            coalesce(p_player_order, '[]'::jsonb)
          ) with ordinality as ordered_player(player_id, ordinality)
        ) as player_position
        where player_position.player_id = current_state.current_player_id;
      end if;
      ...
      if next_current_player_index is not null then
        ...
        next_current_player_id := nullif(
          p_player_order->>next_current_player_index,
          ''
        );
      end if;
  ...
  execute $sql$
    ...
    begin
      ...
      if p_scalar_patch ? 'currentPlayerId' then
        ...
        if next_current_player_id is not null then
          ...
          from (
            select
              ...
              ordinality - 1 as zero_based_index
            from jsonb_array_elements_text(
              coalesce(current_state.state_data->'playerOrder', '[]'::jsonb)
            ) with ordinality as ordered_player(player_id, ordinality)
          ) as player_position
          where player_position.player_id = next_current_player_id;
        ...
        next_current_player_id := nullif(
          current_state.state_data->'playerOrder'->>next_current_player_index,
          ''
        );

        if next_current_player_id is null then
