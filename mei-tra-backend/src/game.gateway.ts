...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private async startTurnAckMonitor(
    ...
    await this.turnMonitorService.startMonitor(
      ...
      async (monitoredRoomId, monitoredSeatId) => {
        await this.roomGameActionQueueService.run(monitoredRoomId, () =>
          this.forceReplacePlayerWithCOM(
            monitoredRoomId,
            monitoredSeatId,
  ...
  private async restoreRoomSession(params: {
    ...
    const result = await this.roomGameActionQueueService.run(roomId, () =>
      this.reconnectionUseCase.execute({
        roomId,
        socketId: client.id,
        authenticatedUser,
      }),
  ...
  async handleTogglePlayerReady(
    ...
    try {
      ...
      const result = await this.roomGameActionQueueService.run(
        data.roomId,
        () =>
          this.togglePlayerReadyUseCase.execute({
            roomId: data.roomId,
            actorSeatId,
          }),
      );
  ...
  async handleChangePlayerTeam(
    ...
    try {
      ...
      const result = await this.roomGameActionQueueService.run(
        data.roomId,
        () =>
          this.changePlayerTeamUseCase.execute({
            roomId: data.roomId,
            actorSeatId,
  ...
  async handleShuffleTeams(
    ...
    try {
      ...
      const result = await this.roomGameActionQueueService.run(
        data.roomId,
        () =>
          this.shuffleTeamsUseCase.execute({
            roomId: data.roomId,
            actorSeatId,
          }),
      );
  ...
  async handleUpdateTeamNames(
    ...
    try {
      ...
      const result = await this.roomGameActionQueueService.run(
        data.roomId,
        () =>
          this.updateTeamNamesUseCase.execute({
            ...data,
            actorSeatId,
          }),
      );
  ...
  async handleFillWithCom(
    ...
    try {
      ...
      const result = await this.roomGameActionQueueService.run(
        data.roomId,
        () =>
          this.fillWithComUseCase.execute({
            roomId: data.roomId,
            actorSeatId,
          }),
      );
  ...
  async handleUpdateAuth(
    ...
    try {
      const currentRoomId = this.playerRooms.get(client.id);
      const executeUpdate = () =>
        this.updateAuthUseCase.execute({
          socketId: client.id,
          token: data?.token,
          currentRoomId,
          handshakeName:
      ...
      const result = currentRoomId
        ? await this.roomGameActionQueueService.run(
            currentRoomId,
            executeUpdate,
          )
        : await executeUpdate();
