...
`src/use-cases/helpers/` は、UseCase 間で共有する小さな純粋関数を置く場所です。ここでいう helper は「便利関数置き場」ではなく、「複数の workflow から同じ意味で呼ばれるルール判定」を 1 箇所に固定するためのものです。

置いてよいもの:

- 複数 UseCase で同じ意味を持つ boolean 判定
- `hasPlayerPassedInBlow()` のように、ゲーム用語で読める純粋なルール predicate
- `countPlayersActedInBlow()` のように、集計基準が分散するとバグになる小さな集計
- player 解決や transport player 変換のように、複数 UseCase で同じ adapter logic を持つもの

置かないもの:

- repository / Supabase / timer / socket emit などの I/O
- `saveState()` や `transitionPhase()` を伴う state mutation
- 1 箇所でしか使わない処理の切り出し
- UseCase の主要な workflow を隠す抽象化
- Service として DI すべき依存を隠す関数

判断基準は「その関数を読めばゲームルール名として意味が通るか」です。単にファイルを短くするための helper 化は避けます。
...
- blow phase 以外の操作を拒否する
- turn check
- pass / declare 済みチェックは `blow-action.helper.ts` の共通判定を使う
- declaration 妥当性検証
...
blow phase の行動済み判定では、`player.isPasser` だけを信じないでください。UI / replay / reconnect / COM autoplay の整合性を守るため、`declarations` と `actionHistory` も同じ基準で見る必要があります。今後 `PassBlowUseCase` 側を触る場合も、同じ helper を使って declare / pass の判定基準を揃えます。
