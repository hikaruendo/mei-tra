...
- プッシュ通知の受け口（通知タップからのルーム復帰）。現在サーバーは通知を送っておらず、アプリも許可を求めない
- 許可済みの端末の通知トークン登録・ログアウト時削除
...
- 通知を許可済み・未許可の端末とシミュレーターでの起動、許可済み端末のトークン登録、ログアウト前のトークン削除
...
- 吹き、ネグリ、Jokerを含むカードプレイ、台札スーツ選択、獲得ペアと得点表示
